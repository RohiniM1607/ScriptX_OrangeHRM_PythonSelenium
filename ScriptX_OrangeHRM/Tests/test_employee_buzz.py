...
class TestBuzz:
    ...
    def test_create_post(self, post_text):
        ...
        self.log.info(
            "Starting test: test_create_post"
        )

        expected_name = get_config(
            "employee_leave",
            "full_name"
        )
        ...
        self.log.info(
            "Navigating to Buzz page"
        )

        actions.navigate_to_buzz_page()

        self.log.info(
            f"Creating post: {post_text}"
        )
        ...
        self.log.info(
            f"Latest post username: {post_user}"
        )

        assert post_user == expected_name, (
            f"Expected username '{expected_name}' "
            f"but got '{post_user}'"
        )

        self.log.info(
            "Create post test passed"
        )
    ...
    def test_edit_post(self, post_text, edit_text):

        self.log.info(
            "Starting test: test_edit_post"
        )
        ...
        self.log.info(
            f"Post text after edit: {actual_text}"
        )

        assert actual_text == edit_text, (
            f"Post text was not updated. "
            f"Expected: '{edit_text}' "
